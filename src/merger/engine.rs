...
impl MergeEngine {
    pub fn new() -> Self {
        Self { default_strategy: MergeStrategy::Replace, field_strategies: IndexMap::new() }
    }

    pub fn with_default_strategy(mut self, strategy: MergeStrategy) -> Self {
        self.default_strategy = strategy; self
    }

    pub fn with_field_strategy(mut self, path: impl Into<Arc<str>>, strategy: MergeStrategy) -> Self {
        self.field_strategies.insert(path.into(), strategy); self
    }

    pub fn get_strategy(&self, path: &str) -> MergeStrategy {
        self.field_strategies.get(path).copied().unwrap_or(self.default_strategy)
    }

    pub fn merge(&self, low: &AnnotatedValue, high: &AnnotatedValue) -> ConfigResult<AnnotatedValue> {
    ...
    fn merge_iterative(low: &AnnotatedValue, high: &AnnotatedValue, strategy: MergeStrategy) -> ConfigResult<AnnotatedValue> {
            ...
            (ConfigValue::Map(l), ConfigValue::Map(_), _) => {
                ...
                for (k, v_high) in r_map.iter() {
                    if let Some(v_low) = result.get_mut(k) {
                        ...
                        let needs_recursive = matches!((&v_low.inner, &v_high.inner),
        ...
        Ok(AnnotatedValue {
            inner: merged, source: high.source.clone(), path: high.path.clone(),
            priority: high.priority.max(low.priority), version: high.version.max(low.version) + 1,
            location: high.location.clone().or(low.location.clone()),
        })
    }

    /// Apply merge strategy to leaf values (non-map).
    fn apply_strategy(low: &AnnotatedValue, high: &AnnotatedValue, strategy: MergeStrategy) -> ConfigResult<AnnotatedValue> {
        let merged = match (&low.inner, &high.inner, strategy) {
            (_, _, MergeStrategy::Replace) => high.inner.clone(),
            (ConfigValue::String(l), ConfigValue::String(r), MergeStrategy::Join { separator }) =>
                ConfigValue::String(format!("{}{}{}", l, separator, r)),
    ...
    pub fn report_conflict(&self, low: &AnnotatedValue, high: &AnnotatedValue) -> Option<ConflictReport> {
        if low.inner == high.inner { return None; }
        Some(ConflictReport {
            path: high.path.clone(),
            low_value: format!("{:?}", low.inner), low_source: low.source.clone(), low_location: low.location.clone(),
            high_value: format!("{:?}", high.inner), high_source: high.source.clone(), high_location: high.location.clone(),
            winner: if high.priority >= low.priority { ConflictWinner::High } else { ConflictWinner::Low },
        })
    }
}
...
pub fn merge_all(values: &[AnnotatedValue], engine: &MergeEngine) -> ConfigResult<AnnotatedValue> {
    if values.is_empty() {
        return Err(ConfigError::ParseError { format: "merge".into(), message: "No values to merge".into(), location: None, source: None });
    }
    let mut r = values[0].clone();
    for v in &values[1..] { r = engine.merge(&r, v)?; }
    Ok(r)
}
...
mod tests {
    ...
    #[test] fn test_engine_new() { assert_eq!(MergeEngine::new().default_strategy, MergeStrategy::Replace); }

    #[test] fn test_merge_replace() {
    ...
    #[test] fn test_merge_append() {
        let e = MergeEngine::new().with_default_strategy(MergeStrategy::Append);
        let l = AnnotatedValue::new(ConfigValue::array(vec![AnnotatedValue::new(ConfigValue::I64(1), SourceId::new("l"), "t.0")]), SourceId::new("l"), "t");
        let h = AnnotatedValue::new(ConfigValue::array(vec![AnnotatedValue::new(ConfigValue::I64(2), SourceId::new("h"), "t.0")]), SourceId::new("h"), "t");
    ...
    #[test] fn test_conflict() {
        ...
        let l = AnnotatedValue::new(ConfigValue::String("low".into()), SourceId::new("l"), "t");
        let h = AnnotatedValue::new(ConfigValue::String("high".into()), SourceId::new("h"), "t");
        assert_eq!(e.report_conflict(&l, &h).unwrap().winner, ConflictWinner::High);
    }
}
