...
mod memory_limit_tests {
    ...
    #[test]
    fn test_memory_limit_loader_default_is_zero() {
...
"#;
        ...
        assert!(
            result.is_ok(),
            "Config should load successfully with high memory limit"
        );
    }
...
"#;
        ...
        let current_memory_mb = if let Some(process) = sys.process(current_pid) {
            process.memory() as f64 / 1024.0 / 1024.0
        } else {
            0.0
        };

        let low_limit = if current_memory_mb > 0.0 {
            (current_memory_mb as usize).saturating_sub(1)
        } else {
            1
        };
        ...
        let loader: ConfigLoader<SimpleConfig> = ConfigLoader::new()
            .with_file(&config_path)
            .with_memory_limit(low_limit);

        let result = loader.load().await;

        eprintln!(
            "Current memory: {} MB, Low limit: {} MB",
            current_memory_mb, low_limit
        );
        eprintln!("Result: {:?}", result);

        if current_memory_mb > 0.0 && current_memory_mb as usize > low_limit {
            assert!(
                result.is_err(),
                "Config load should fail when memory exceeds limit"
            );
            if let Err(confers::ConfigError::MemoryLimitExceeded { limit, current }) = result {
                assert_eq!(limit, low_limit);
...
"#;
        ...
        let realistic_limit = if current_memory_mb > 0.0 {
            (current_memory_mb as usize) + 1000
        } else {
            2000
        };
        ...
        let loader: ConfigLoader<SimpleConfig> = ConfigLoader::new()
            .with_file(&config_path)
            .with_memory_limit(realistic_limit);
        ...
        assert!(
            result.is_ok(),
            "Config should load successfully with realistic memory limit (PRD requirement)"
        );
    }
...
"#;
        ...
        assert!(
            result.is_ok(),
            "Config should load successfully with realistic memory limit"
        );
    ...
    fn test_memory_limit_validation_edge_cases() {
        ...
        let loader3: ConfigLoader<SimpleConfig> = ConfigLoader::new().with_memory_limit(0);
...
"#;
        ...
        for i in 0..5 {
            ...
            assert!(
                result.is_ok(),
                "Config load {} should succeed with reasonable memory limit",
                i
            );
        }
    }
