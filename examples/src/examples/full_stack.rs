...
use confers::secret::{derive_field_key, SecretString, SecretKeyProvider, XChaCha20Crypto};
...
fn demo_encryption() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Encrypted: nonce={:?}, ciphertext len={}", nonce, ciphertext.len());
...
fn demo_watcher() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Watcher config: debounce={}ms, min_interval={}ms, max_failures={}",
